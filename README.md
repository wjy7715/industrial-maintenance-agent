...
- 不是伪造工业落地：仿真数据、公开数据和维修资料边界明确标注。
- 不是只展示效果：固定 30 个检索案例，16 项自动化测试。
...
```powershell
cd "C:\Users\Administrator\Documents\Personal assistant\industrial-maintenance-agent"
$env:PYTHONPATH = "src"

# 诊断演示
..\.venv\Scripts\python.exe -m industrial_maintenance_agent.cli diagnose `
  --equipment-id PUMP-001 `
  --symptom "泵振动明显，出口压力下降"

# 30 案例评测
..\.venv\Scripts\python.exe -m industrial_maintenance_agent.cli evaluate
...
| 自动化测试 | 16/16 | 领域、仓储、工具、编排、数据、安全、网页和 Hermes 适配 |
