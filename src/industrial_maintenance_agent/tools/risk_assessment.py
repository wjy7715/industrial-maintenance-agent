...
class RiskAssessmentTool:
    ...
    def run(self, equipment_type: str, telemetry: dict[str, Any]) -> dict[str, Any]:
        ...
        for field, (warning, critical) in self.PUMP_THRESHOLDS.items():
            ...
            if value >= critical:
                level = "critical"
                signals.append(f"{field}={value} 达到演示临界阈值 {critical}")
            elif value >= warning:
                if level != "critical":
                    level = "high"
                signals.append(f"{field}={value} 超出演示告警阈值 {warning}")
        return {
            "level": level,
            "signals": signals,
